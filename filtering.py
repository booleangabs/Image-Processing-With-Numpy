...
def boxBlur(image: np.ndarray, size: int=3) -> np.ndarray:
    kernel = np.ones((size, size))
    return convolve(image, kernel)

def gaussianBlur(image: np.ndarray, sigma: float, size: int=3) -> np.ndarray:
    ksm1o2 = (size - 1) / 2
    sub = (-1 / (2 * sigma**2))
...
def medianBlur(image: np.ndarray, size: int=3) -> np.ndarray:
...
def sharpen(image: np.ndarray, threshold: float, sigma: float, alpha: float=0.33333, size: int=3) -> np.ndarray:
...
def sobel(image: np.ndarray, size: int=3) -> np.ndarray:
...
def convolve(image: np.ndarray, kernel: np.ndarray, flipped: bool=True) -> np.ndarray:
    result = np.zeros_like(image)
    filter_ = kernel.copy()
    ks = filter_.shape[0]
    c = ks // 2
